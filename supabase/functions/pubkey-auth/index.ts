...
// POST { publicKeyJwk, signature, nonce } -> verifica que `signature` sea una firma ECDSA
// válida de `nonce` hecha con la clave privada correspondiente a `publicKeyJwk`. Si es así,
// busca (o crea) el auth.users asociado al hash de esa clave pública y devuelve un token_hash
// que el cliente puede canjear con supabase.auth.verifyOtp({ token_hash, type: 'email' })
// para obtener una sesión real. No se envía ningún email de verdad: generateLink solo emite
// el token, nunca dispara el proveedor de correo.
